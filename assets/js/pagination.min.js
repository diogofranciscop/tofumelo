...
`;$("head").append(o)}function createPaginationButtons(a,e,t){let n=Math.ceil(e.length/t);$("#pagination-container").empty();let i=$("<button>").text("PRIMEIRA P\xc1GINA").addClass("pagination-button").on("click",()=>{loadPage(0,e),$(window).scrollTop(0)}),o=$("<button>").text("\xdaLTIMA P\xc1GINA").addClass("pagination-button").on("click",()=>{loadPage(n-1,e),$(window).scrollTop(0)}),s=$("<button>").text("←").addClass("pagination-button").on("click",()=>{loadPage(Math.max(0,a-1),e),$(window).scrollTop(0)}),d=$("<button>").text("→").addClass("pagination-button").on("click",()=>{loadPage(Math.min(n-1,a+1),e),$(window).scrollTop(0)});$("#pagination-container").append(i,s),a>2&&$("#pagination-container").append($("<span>").text("1").addClass("pagination-button").on("click",()=>loadPage(0,e))),a>3&&$("#pagination-container").append($("<span>").text("...").addClass("pagination-ellipsis"));for(let p=Math.max(0,a-1);p<=Math.min(n-1,a+1);p++){let r=$("<button>").text(p+1).addClass("pagination-button");p===a&&r.addClass("active"),r.on("click",()=>{loadPage(p,e),$("html, body").animate({scrollTop:0},400,"linear")}),$("#pagination-container").append(r)}a<n-4&&$("#pagination-container").append($("<span>").text("...").addClass("pagination-ellipsis")),a<n-3&&$("#pagination-container").append($("<span>").text(n).addClass("pagination-button").on("click",()=>loadPage(n-1,e))),$("#pagination-container").append(d,o)}document.addEventListener("DOMContentLoaded",function(){document.getElementById("loader")});