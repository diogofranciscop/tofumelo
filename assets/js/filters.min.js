...
`;l.append(i);let s=l.find(`img[src="${imagePath}"]`);s.on("load",function(){let e=$(this).closest(".card");e.find(".skeleton-card").addClass("hidden"),$(this).fadeIn(),e.find(".title-card").removeClass("hidden").fadeIn(),e.find(".new-tape").removeClass("hidden").fadeIn()})}),updatePaginationButtons(e,t.length,s),window.paginationTriggered&&setTimeout(()=>{window.scrollTo({top:0,behavior:"smooth"}),window.paginationTriggered=!1},0)}function loadPosts(e){let t=window.innerWidth<=768,i=t?10:20,s=(e-1)*i,o=filteredPosts.slice(s,s+i),n=document.getElementById("post-container");n.innerHTML="",o.forEach(e=>{let t=`
...
`;n.insertAdjacentHTML("beforeend",t)}),updatePaginationButtons(e,filteredPosts.length,i)}function updatePaginationButtons(e,t,i){let s=Math.ceil(t/i),o=document.getElementById("pagination-container");o.innerHTML="";let n=document.createElement("button");n.textContent="PRIMEIRA P\xc1GINA",n.classList.add("pagination-button"),n.addEventListener("click",()=>{window.paginationTriggered=!0,loadPaginatedPosts(1,filteredPosts)}),o.appendChild(n);for(let l=Math.max(1,e-2);l<=Math.min(s,e+2);l++){let a=document.createElement("button");a.textContent=l,a.classList.add("pagination-button"),l===e&&a.classList.add("active"),a.addEventListener("click",()=>{window.paginationTriggered=!0,l!==e&&loadPaginatedPosts(l,filteredPosts)}),o.appendChild(a)}let d=document.createElement("button");d.textContent="\xdaLTIMA P\xc1GINA",d.classList.add("pagination-button"),d.addEventListener("click",()=>{window.paginationTriggered=!0,loadPaginatedPosts(s,filteredPosts)}),o.appendChild(d)}function setupFilterButton(){$(".filter-button").on("click",function(e){e.stopPropagation(),$("#filterBox").toggle(),function e(t){let i=$(t).offset(),s=$(t).outerHeight();$("#filterBox").css({top:i.top+s+10+"px",left:i.left+"px",position:"absolute"})}(this)}),$(document).on("click",function(e){let t=$("#filterBox"),i=$(".filter-button");t.is(e.target)||0!==t.has(e.target).length||i.is(e.target)||t.hide()})}function setupRoleSelection(){$(".button-4").on("click",function(){let e=$(this).data("role");toggleSelection(e,"role"),$(this).toggleClass("selected",selectedRoles.includes(e)),filterAndSortPosts()})}function setupDietSelection(){$("#filterBox input[data-diet]").on("change",function(){let e=$(this).data("diet")||"";toggleSelection(e,"diet"),filterAndSortPosts()})}function setupSorting(){$('input[type="checkbox"][name="sort"]').on("change",function(){let e=$(this).attr("id");$(this).is(":checked")?(selectedTitleSort=e.startsWith("toggleOrdem")?e:"",selectedTimeSort=e.startsWith("toggleTempo")?e:"",$('input[type="checkbox"][name="sort"]').not(this).prop("checked",!1)):"toggleOrdemAZ"===e||"toggleOrdemZA"===e?selectedTitleSort="":("toggleTempoMenor"===e||"toggleTempoMaior"===e)&&(selectedTimeSort=""),filterAndSortPosts()})}function setupSearch(){$(".searchTerm").on("input",function(){searchTerm=$(this).val().toLowerCase(),filterAndSortPosts()})}function toggleSelection(e,t){let i="role"===t?selectedRoles:selectedDiets,s=i.indexOf(e);-1===s?i.push(e):i.splice(s,1)}